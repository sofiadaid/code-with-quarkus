        ...
        String html = """
                ...
                <style>
                    * { box-sizing: border-box; margin: 0; padding: 0; }
                    ...
                    input[type=text], input[type=file], textarea {
                        ...
                        background: white; color: #1a1a1a; margin-bottom: 10px;
                    }
                    ...
                    .status { font-size: 13px; margin-top: 10px; padding: 8px 12px; border-radius: 6px; }
                    ...
                    th { text-align: left; padding: 6px 10px; font-size: 12px; color: #888; border-bottom: 1px solid #eee; }
            ...
            <body>
               ...
               <!-- 1 — CRÉER UNE TABLE -->
                ...
                <!-- APERCU TABLE -->
                        <div class="card">
                            <h2>Aperçu de la table</h2>
                            <label>Nom de la table</label>
                            <input type="text" id="previewTable" placeholder="ex: taxi" />
                            <div style="display:flex; gap:10px; align-items:center; margin-bottom:10px;">
                                <label style="margin:0;">Lignes à afficher</label>
                                <select id="previewLimit" style="padding:6px 10px; border:1px solid #ddd; border-radius:6px; font-size:13px;">
                ...
                <div class="card">
                    ...
                    <input type="text" id="queryTable" placeholder="ex: taxi" />
                ...
                <script>
                    ...
                    async function doPreview() {
                                const tableName = document.getElementById('previewTable').value.trim();
                                ...
                                try {
                                    // 1. Vrais noms de colonnes si la table existe
                                    let colNames = null;
                                    if (tableName) {
                                        const schemaRes = await fetch(`/api/tables/${tableName}`);
                                        if (schemaRes.ok) {
                                            const schema = await schemaRes.json();
                                            if (schema.columns && schema.columns.length > 0) {
                                                colNames = schema.columns.map(c => c.name);
                                            }
                                        }
                                    }
                                    ...
                                    // Fallback si pas de table connue
                                    if (!colNames) {
                                        colNames = rows[0]?.map((_, i) => "col_" + i) || [];
                                    }
                
                                    renderPreview(colNames, rows, tableName || "preview");
                            ...
                            function renderPreview(colNames, rows, tableName) {
                                if (!rows.length) {
                                    setStatus('previewStatus', 'Table vide ou introuvable.', 'info');
                                    return;
                                }
                    ...
                    async function doQuery() {
                        ...
                        if (!table || !q) {
                            setStatus('queryStatus', 'Remplissez la table et la requête.', 'err');
                            return;
                        }
